...
class ExampleData(Dataset):
    ...
    out_path = os.path.join('./output_data', 'Example_data')
    ...
    def prepare_data(self, user_id, window_sec):
        path = os.path.join(self.dirpath, f"subject_{str(user_id)}", f"{user_id}_part1_{self.device}_labelled.csv")

        try:
            df_eeg = pd.read_csv(path)
        except FileNotFoundError:
            path = os.path.join(self.dirpath, f"subject_{str(user_id)}", f"{user_id}_part2_{self.device}_labelled.csv")
            df_eeg = pd.read_csv(path)
